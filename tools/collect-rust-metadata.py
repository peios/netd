#!/usr/bin/env python3
"""Collect the exact licence closure of the shipped Netd executables."""
...
def reachable_packages(data: dict) -> list[dict]:
    ...
    roots = {
        ...
        if package["source"] is None and package["name"] in {"netd", "net"}
    }
    seen: set[str] = set()
    pending = list(roots)
    while pending:
...
def fallback_notice(package: dict) -> pathlib.Path | None:
    source = package.get("source") or ""
    if source.startswith("git+https://github.com/peios/peios-rs.git"):
        return pathlib.Path("third-party/peios-rs-LICENSE")
    if package["name"] == "pnp-core" and source.startswith(
        "git+https://github.com/peios/pkm.git"
    ):
        return pathlib.Path("third-party/pnp-core-LICENSE")
    return None
...
def main() -> None:
    ...
    for package in reachable_packages(metadata()):
        ...
        # Workspace crates share the release's root MIT notice. Registry and
        # git crates must contribute the notice that accompanies their source.
        ...
        if not notices:
            fallback = fallback_notice(package)
            notices = [fallback] if fallback is not None else []
        if not notices:
            raise SystemExit(
